...
auto Full_Division::evaluate(stack_type& stack) const -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for //"};
    }
...
auto Logarithm::evaluate(stack_type& stack) const -> void
{
    if (stack.size() < 1) {
...
auto main(int argc, char* argv[]) -> int
{
    ...
    for (auto const& each : make_args(argc, argv)) {
        try {
            ...
            using student::rpn_calculator::Full_Division;
            ...
            if (each == "p") {
                ...
                calculator.push(std::make_unique<Full_Division>());
            } else if (each == "%") {
